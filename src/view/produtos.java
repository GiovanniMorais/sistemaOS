...
public class produtos extends JDialog {
	...
	public produtos() {
		...
		lblFoto.setIcon(new ImageIcon(produtos.class.getResource("/img/simbolo-de-interface-de-camera-fotografica-para-botao.png")));
		...
		lblNewLabel_1.setIcon(new ImageIcon(produtos.class.getResource("/img/codigo-de-barras.png")));
		...
		if (txtCodigo.getText().isEmpty()) {
			...
			try {
				...
				if (rs.next()) {
					...
					// passo 2: formatar a data para o JCalendar
					Date dataEntrada = new SimpleDateFormat("yyyy-MM-dd").parse(setarDataEnt);
					// passo 3 : exibir o resultado do JCalendar
					dateEntrada.setDate(dataEntrada);
					String setarDataVal = rs.getString(9);
					Date dataValidade = new SimpleDateFormat("yyyy-MM-dd").parse(setarDataVal);
					dateValidade.setDate(dataValidade);
	...
	private void pesquisarProdutoBarcode() {
		if (txtBarcode.getText().isEmpty()) {
			...
			String readCodigo = "select * from produtos inner join fornecedores on produtos.idfor = fornecedores.idfor where barcode = ?";
			try {
				con = dao.conectar();
				pst = con.prepareStatement(readCodigo);
